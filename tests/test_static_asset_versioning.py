...
def test_cloud_mail_console_template_uses_versioned_static_assets():
    template = Path("templates/cloud_mail_console.html").read_text(encoding="utf-8")

    assert '/static/css/style.css?v={{ static_version }}' in template
    assert '/static/js/utils.js?v={{ static_version }}' in template
    assert '/static/js/cloud_mail_console.js?v={{ static_version }}' in template
